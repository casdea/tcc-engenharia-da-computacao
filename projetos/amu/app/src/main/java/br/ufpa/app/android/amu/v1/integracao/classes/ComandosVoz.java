package br.ufpa.app.android.amu.v1.integracao.classes;

public class ComandosVoz {
    public static final int COMANDO_NAO_RECONHECIDO = -1;
    public static final int LISTA_MEDICAMENTOS = 1;
    public static final int DESCREVA_MEDICAMENTO = 2;
    public static final int DESCREVA_HORARIO = 3;
    public static final int TELA_ANTERIOR = 4;
    public static final int DOSE_REALIZADA = 5;
    public static final int ESTOQUE_ATUAL = 6;
    public static final int ENTRADA_ESTOQUE = 7;
    public static final int SAIDA_ESTOQUE = 8;
    public static final int ALTERNAR_PERFIL = 9;
    public static final int SAIR = 10;
    public static final int COMANDOS_DISPONIVEL_TELA = 11;
    public static final int COMANDOS_TELA_ATUAL = 12;
    public static final int PESQUISAR_MEDICAMENTOS_ANVISA = 13;
}

